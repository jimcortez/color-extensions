...
vec4 get_histogram_color() {
    float fNumColors = floor(numColors);
    int iNumColors = int(fNumColors);
    float lineHeight = 1.0 / fNumColors; // Height for each histogram segment
    float accumulatedHeight = 0.0; // Tracks vertical position for histogram sections

    for (int i = 0; i < 50; i++) {
        if (i >= iNumColors) break;

        // Get the quantized color from buffer1
        vec2 clusterUV = vec2(float(i) / (fNumColors - 1.0), 0.5);
        vec4 clusterColor = IMG_NORM_PIXEL(buffer1, clusterUV);

        // `clusterColor.a` contains the normalized ratio for this color
        float ratio = clusterColor.a;

        // Determine if the current fragment lies within the range for this histogram line
        float colorTop = accumulatedHeight + ratio * lineHeight;
        if (isf_FragNormCoord.y <= colorTop) {
            return vec4(clusterColor.rgb, 1.0);
        }

        // Move to the next color's position
        accumulatedHeight = colorTop;
    }
...
vec4 get_animation_color() {
    ...
    vec4 dominantColor = IMG_NORM_PIXEL(buffer1, vec2(0.0, 0.5));
...
vec4 main_pass1() {
    ...
    if (uv.x < 0.5 && uv.y > 0.5) {
        ...
        return IMG_NORM_PIXEL(inputImage, imageUV);
//       return IMG_NORM_PIXEL(colorCounter, imageUV);
    } else if (uv.x >= 0.5 && uv.y > 0.5) {
        vec2 newUV = vec2((uv.x - 0.5) * 2.0, uv.y * 2.0 - 1.0);
        return IMG_NORM_PIXEL(buffer1, newUV);
    } else if (uv.x < 0.5 && uv.y <= 0.5) {
...
void main() {
    if (PASSINDEX == 0) {
        gl_FragColor = main_pass0_kmeans();
    } else {
