/*{
  ...
  "INPUTS" : [
    ...
    },
    {
      ...
      "DEFAULT" : 1.0,
      "MIN" : 0.0,
      "LABEL" : "Speed at which camera pans up and down"
    }
  ],
...
void main() {
    ...
    vec3 cameraPosition = vec3(0.0 + horizontalOffset, 0.25 + 0.25 * cos(panSpeed * TIME) * 1.0, 0.0);
    ...
    if (!hitLine && uv.y > lastHeight / RENDERSIZE.y) {
        ...
        gl_FragColor =  vec4(smoothstep(1.0, 0.0, uv.y) * foregroundColor.rgb, 1.0);
    } else {
        gl_FragColor = vec4(sqrt(clamp(accumulatedColor, 0.0, 1.0)), 1.0);
    }
}
